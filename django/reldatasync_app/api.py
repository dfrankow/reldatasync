...
def _get_datastore(
    datastore_name: str, table_name: str
) -> Datastore | tuple[int, dict]:
    try:
...
@api.get(
    "{datastore}/{object_name}/doc/{docid}", response={200: dict, 403: ErrorSchema}
)
def get_doc(
    ...
    if not table:
        return 403, {"message": f"Unknown table '{object_name}'"}
    with _get_datastore(datastore, table) as datastore1:
        if isinstance(datastore1, tuple):
            return datastore1
...
def post_doc(request, datastore: str, table: str, increment_rev: bool = False):
    ...
    """
    datastore1 = _get_datastore(datastore, table)
    if isinstance(datastore1, tuple):
        return datastore1
